...
3. Completed on 2026-09-19: all 423 validated `productId|variantId|shade|texture` keys are mapped in `lib/bm/shopify-variant-map.ts`. Preserve this verified mapping; change it only after a confirmed Shopify variant structure change. It contains public identifiers, not secrets. No `BM_SHOPIFY_VARIANTS` environment variable is needed. API secrets must remain server-side in the hosting provider's runtime settings.
